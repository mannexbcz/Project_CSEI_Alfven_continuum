...
% elongated and shifted equilibriuum. The computation time with respect to
% these two parameters is also computed.
% -------------------------------------------------------------------------
%%
...
time_bands = zeros(1,length(bands)); % computation times
...
namefig=['exact_error_gap_fct_nb_bandes'];
path='C:\Users\manon\Desktop\projet CSE I\figures\elongation\';
gaps=real(gaps);
diff=gaps-gaps(end);

p=polyfit(bands,log(abs(diff)),1);
pval=polyval(p,bands);
...
%% Figure for the computation time 

save=0; %Set 1 to save the figure, 0 otherwise
namefig=['time_bands_exact'];
path='C:\Users\manon\Desktop\projet CSE I\figures\elongation\';
...
save=0; %Set 1 to save the figure, 0 otherwise
namefig=['exact_error_gap_fct_rang'];
path='C:\Users\manon\Desktop\projet CSE I\figures\elongation\';
